...
async def start(client, message):
    ...
    if AUTH_CHANNEL and not await is_subscribed(client, message):
        try:
            ...
            return
        try:
            btn = [[InlineKeyboardButton("ᴊᴏɪɴ ɴᴏᴡ", url=invite_link.invite_link)]]
            ...
            if REQUEST_TO_JOIN_MODE == True:
                if TRY_AGAIN_BTN == True:
                    text = "**❌ ʏᴏᴜ ʜᴀᴠᴇ ɴᴏᴛ ᴊᴏɪɴᴇᴅ ᴏᴜʀ ᴍᴏᴠɪᴇ ᴜᴘᴅᴀᴛᴇ ᴄʜᴀɴɴᴇʟ...\n\n ᴀꜰᴛᴇʀ ᴊᴏɪɴɪɴɢ ᴄʟɪᴄᴋ ᴏɴ ᴛʀʏ ᴀɢᴀɪɴ ʙᴜᴛᴛᴏɴ ☟**"
                else:
        ...
        if vj and PREMIUM_AND_REFERAL_MODE == True:
            ...
            if num_referrals == REFERAL_COUNT:
                ...
                if seconds > 0:
                    ...
                    user_data = {"id": user_id, "expiry_time": expiry_time, "reminder_3days_sent": False, "expiry_reminder_sent": False} 
...
async def remove_premium_cmd_handler(client, message):
    ...
    if len(message.command) == 2:
      ...
      #  time = message.command[2]
        ...
        if seconds > 0:
            expiry_time = datetime.datetime.now() + datetime.timedelta(seconds=seconds)
            user_data = {"id": user_id, "expiry_time": expiry_time, "reminder_3days_sent": False, "expiry_reminder_sent": False}  # Using "id" instead of "user_id"
            await db.update_user(user_data)  # Use the update_user method to update or insert user data
