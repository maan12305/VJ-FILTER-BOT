...
async def verify_user(bot, userid, token):
    ...
    await bot.send_message(LOG_CHANNEL,f"DEBUG: Saved verification for {user.id}")
...
async def check_verification(bot, userid):
    ...
    user_data = await db.col.find_one({"id": int(user.id)})

    if not user_data:
        return False

    verified_date = user_data.get("verified_date")

    if verified_date == str(date.today()):
        return True
